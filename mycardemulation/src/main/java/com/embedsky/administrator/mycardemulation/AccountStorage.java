...
public class AccountStorage {
    ...
    public static void SetPassword(Context c, String s){
        synchronized(sAccountLock) {
            Log.i(TAG, "Setting password number: " + s);
            SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(c);
            prefs.edit().putString(PREF_ACCOUNT_PASSWORD, s).commit();
            sPassword = s;
        }
    }
    public static String GetPassword(Context c) {
        synchronized (sAccountLock) {
            if (sPassword == null) {
                SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(c);
                String password = prefs.getString(PREF_ACCOUNT_PASSWORD, DEFAULT_ACCOUNT_PASSWORD);
                sPassword = password;
            }
            return sPassword;
        }
