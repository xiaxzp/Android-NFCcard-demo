...
public class CardService extends HostApduService {
    ...
    public static String toHex(byte[] buf) {
        if (buf == null)
            return "";
       // Log.i(TAG, "encode: " + new String(buf));
    ...
    public byte[] processCommandApdu(byte[] commandApdu, Bundle extras) {
       ...
       MainActivity.sb.append("Received APDU: "+ByteArrayToHexString(commandApdu)+"\n");
        MainActivity.dataHandler.sendEmptyMessage(1);//传送改变文本的信息
        // If the APDU matches the SELECT AID command for this service,
        // send the loyalty card account number, followed by a SELECT_OK status trailer (0x9000).
        try {                                                                                       //此处处理要传输的信息，检测自动传输
            if (Arrays.equals(SELECT_APDU, commandApdu)) {
                String account = AccountStorage.GetAccount(this);
                String pwheader = new String(PASSWORD_HEADER);
                String password = AccountStorage.GetPassword(this);
                String all1 = account + pwheader + password;
                String all = encrypt(AES_PASSWORD,all1);//new String(payload, "UTF-8");
                byte[] accountBytes = all.getBytes();

                Log.i(TAG, "Sending account number: " + all);Log.i(TAG, "encode: " + new String(accountBytes, "UTF-8"));
                MainActivity.sb.append("Sending account number: " + all + "\n");
